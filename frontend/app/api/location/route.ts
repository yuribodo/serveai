...
function json(body: { label: string } | { error: string }, status = 200) {
...
export async function POST(request: Request) {
  ...
  if (cached && cached.expiresAt > Date.now()) return json({ label: cached.label });
  ...
  try {
    const result = await queueNominatimRequest(async () => {
      ...
      const response = await fetch(endpoint, {
        headers: {
          ...
          Referer: url.origin,
          "User-Agent": `ServeAI/0.1 (${url.origin})`,
        },
    ...
    return json({ error: "Não foi possível identificar a localização." }, 502);
  }
}
