...
async def test_requirements_understand_urgent_natural_language_in_one_turn() -> None:
    ...
    assert "emperrou" in request.problem
    assert request.budget is not None
    assert request.budget.maximum == 100
    assert request.availability[0].start == now
    assert request.availability[0].end == now.replace(hour=19)


@pytest.mark.asyncio
async def test_requirements_understand_as_soon_as_possible() -> None:
    now = datetime(2026, 8, 19, 15, tzinfo=ZoneInfo("America/Sao_Paulo"))

    request = await RuleBasedRequirementsExtractor().extract(
        ServiceRequestData(service_type="chaveiro", problem="A porta não abre"),
        [("user", "quero o mais rápido possível")],
        now,
    )
