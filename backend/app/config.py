...
class Settings(BaseSettings):
    ...
    # Explicit escape hatch for the public hackathon demo. It enables only the
    # deterministic/demo adapters and must never be confused with a live setup.
    ...
    def validate_runtime_safety(self) -> Settings:
        if self.demo_deployment:
            if self.has_google_places or self.has_resend or self.has_google_calendar:
                raise ValueError("DEMO_DEPLOYMENT não pode ser combinado com integrações externas")
            if self.has_openai and self.has_ai_gateway:
                raise ValueError("Configure OPENAI_API_KEY ou AI Gateway, não ambos")
            return self
    ...
    def has_ai_gateway(self) -> bool:
        return bool(self.ai_gateway_enabled and (self.ai_gateway_api_key or self.vercel_oidc_token))
