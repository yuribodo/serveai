...
import {
  ...
  type EditableField, type FlowStage, type RequestLocation, type ServiceRequest,
} from "@/lib/flow";
...
function ComposerDock({ children, dockRef }: { children: ReactNode; dockRef?: Ref<HTMLDivElement> }) {
  return <div className="composer-dock" ref={dockRef}>{children}<p>O ServeAI pode cometer erros. Confirme informações importantes.</p></div>;
}
...
type CollectionQuestion = "location" | "problem" | "budget" | "ready";
    ...
    <AgentMessage>
      ...
      <div className="activity-card" aria-live="polite" aria-label={`Progresso: ${currentLabel}`}>
        ...
        <div className="activity-list">{activitySteps.map((step, index) => {
          ...
          return <div className={`activity-row is-${status}`} key={step.label}><span className="activity-status"><ActivityIcon status={status} /></span><div className="activity-copy"><span>{step.label}</span>{detail && <small>{detail}</small>}</div><time>{status === "done" && step.time && <span className="activity-time-copy" key={step.time}>{step.time}</span>}</time></div>;
        })}</div>
...
function ResultContent({ request, detailsOpen, onToggleDetails }: { request: ServiceRequest; detailsOpen: boolean; onToggleDetails: () => void }) {
  ...
  return (
    <AgentMessage>
      ...
      <div className="confirmation-card result-reveal">
        <span><Check size={15} /></span>
        <div className="confirmation-content"><strong>Agendamento confirmado.</strong><p>A confirmação foi enviada ao profissional.</p></div>
      </div>
      <a className="calendar-button pressable result-reveal" href={calendarUrl} target="_blank" rel="noreferrer">
        <span className="calendar-app-icon" aria-hidden="true"><img src="/google-calendar.svg" alt="" /></span>
        <span className="calendar-button-copy"><strong>Adicionar ao Google Calendar</strong><small>Hoje, 15:30–16:30 · pronto para salvar</small></span>
...
function ActiveRequestScreen({ stage, originalRequest, request, phase, onUpdate, onUpdateLocation, onBegin, onAdjust, onReset }: {
  stage: ActiveStage;
  originalRequest: string;
  request: ServiceRequest;
  phase: number;
  onUpdate: (field: EditableField, value: string) => void;
  onUpdateLocation: (location: RequestLocation) => void;
  onBegin: () => void;
  onAdjust: () => void;
  onReset: () => void;
}) {
  ...
  const question: CollectionQuestion = !request.location ? "location" : !request.problem ? "problem" : !request.budget ? "budget" : "ready";
  const answer = (value: string) => {
    followsLatestRef.current = true;
    if (question === "location") onUpdate("location", value);
  ...
  useEffect(() => {
    ...
    const firstFrame = window.requestAnimationFrame(() => {
      secondFrame = window.requestAnimationFrame(() => {
        ...
        settleTimer = window.setTimeout(() => { autoScrollingRef.current = false; }, reduceMotion ? 0 : 420);
      });
    });
    return () => {
    ...
    <section className={`conversation-screen active-flow-screen stage-panel ${stage === "result" ? "result-screen" : ""}`} aria-label="Conversa">
      <div className="conversation-thread">
        ...
        <div className="flow-transcript">
          ...
          {stage !== "collect" && <div className="thread-stage-entry is-work"><WorkContent request={request} phase={phase} complete={stage === "result"} onAdjust={onAdjust} /></div>}
          {stage === "result" && <div className="thread-stage-entry is-result"><ResultContent request={request} detailsOpen={detailsOpen} onToggleDetails={() => { followsLatestRef.current = true; setDetailsOpen((open) => !open); }} /></div>}
      ...
      <ComposerDock dockRef={composerDockRef}>
        <div className="dock-state" key={dockKey}>
          {stage === "collect" && question !== "ready" && <Composer value={reply} onChange={setReply} onSubmit={submitReply} onLocation={(location) => { followsLatestRef.current = true; onUpdateLocation(location); }} placeholder={question === "location" ? "Digite seu endereço, bairro ou CEP" : "Responda ao ServeAI"} quiet />}
          {stage === "collect" && question === "ready" && <div className="ready-dock"><Check size={15} />Revise os detalhes ou comece a busca acima</div>}
          {stage === "work" && <div className="waiting-composer"><CircleDashed className="activity-spinner" size={16} />ServeAI está trabalhando na sua solicitação</div>}
